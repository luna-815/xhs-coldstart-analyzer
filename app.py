"""
Cold Start Lab — 小红书冷启动分析（Streamlit）
...
from llm_client import LlmUserError, call_llm_backend_stream
...
【Markdown 标题与符号（必须严格遵守，避免“# 号直出/乱码”）】
1) 报告正文仅允许使用 `###` 作为标题符号；禁止 `##`、禁止 `####`、禁止 `**` 加粗、禁止行内 `###` 直接作为普通字符出现（除真实标题行外不要写 # 号）。
2) 六大模块大标题必须统一为 `### 一、...` 到 `### 六、...`（与示例如下完全同构，只改内容，不要加任何其它 # 级标题）。
3) 模块内小标题用纯中文行首短句 + 冒号，不用 `####`；例如 `1) 节奏：...` 或 `关键风险：...`（不要加粗）。
4) 正文用短句 + 无序列表（以 `-` 开头），不要长段落铺陈；不要出现任何反引号代码围栏。
...
def _make_printable_html(snapshot: Dict[str, Any], summary: Dict[str, Any], report_md: str) -> str:
    ...
    """
    title = str(snapshot.get("brand_name") or snapshot.get("category") or "Cold Start Lab").strip()
    created = datetime.now().strftime("%Y-%m-%d %H:%M")
    score = summary.get("score", "")
    rating = summary.get("rating_label", "")
    body_html = _markdown_to_safe_html(report_md or "")
    css = """
<style>
  ...
  .report-md { margin-top:14px; font-size:13px; line-height:1.75; }
  .report-md h3 { font-size:15px; margin:14px 0 8px; color:#7a6328; font-weight:700; }
  .report-md ul { padding-left:18px; margin:8px 0; }
  .report-md li { margin:6px 0; }
...
"""
    ...
    return f"<!doctype html><html><head><meta charset='utf-8'>{css}</head><body>{head}{badge}<div class='hr'></div><div class='report-md'>{body_html}</div></body></html>"
...
def _clean_full_report_markdown(md: str) -> str:
    ...
    for ln in lines:
        ...
        if not t:
            ...
            continue
        # 去掉标题噪声行（只剩下 #）
        if set(t) <= {"#"} and t.startswith("#"):
            continue

        # 标题层级收敛：只允许 ###
        if t.startswith("####"):
            t = re.sub(r"^#{4,}\\s*", "", t).strip()
        if t.startswith("### "):
            pass
        elif t.startswith("## "):
            t = "### " + t[3:].strip()
...
def _markdown_to_safe_html(md: str) -> str:
    ...
    if not md:
        return ""
    cleaned = _clean_full_report_markdown(md)
...
def _format_dimension_line(dim_name: str, score: Any, comment: str) -> str:
    ...
    if m:
        deduct = m.group(2).strip()
        rest = cm[m.end() :].strip()
        m2 = re.search(r"(优化|改进)[:：](.+)$", rest)
        if m2:
            improve = m2.group(2).strip()
        else:
            improve = rest.strip("；。 ")
    else:
        improve = cm
...
<html>
  <head>
    ...
    <style>
      ...
      .report-md h3 {{ font-size: 15px; margin: 12px 0 8px; color:#7a6328; font-weight: 800; }}
      .report-md ul {{ padding-left: 18px; margin: 8px 0; }}
      .report-md li {{ margin: 6px 0; line-height: 1.75; font-size: 13px; color:#111; }}
      .report-md p {{ margin: 8px 0; line-height: 1.75; font-size: 13px; color:#111; }}
    </style>
  </head>
  <body>
    <div class="wrap">
...
def _streaming_report_tail(raw: str) -> str:
    """从完整模型输出中提取「摘要块之后」的报告正文片段（可能为空/不完整）。"""
    end_tag = "<<<END_SUMMARY_JSON>>>"
    s1 = raw.find(end_tag)
    if s1 == -1:
        return ""
    return raw[s1 + len(end_tag) :]
...
def render_home_page(sb: Optional[Any]) -> None:
    st.markdown(
        '<div class="herti-eyebrow">COLD START LAB<br/>— a growth strategy map —</div>',
        unsafe_allow_html=True,
    )
    st.markdown('<div class="herti-title">Cold Start Lab</div>', unsafe_allow_html=True)
    st.markdown('<div class="herti-subtitle">小红书商家冷启动分析</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="herti-lead">流量藏在细节里，<br/>但你的产品里，有一套专属的增长逻辑，<br/>正等待被精准激活。</div>',
        unsafe_allow_html=True,
    )
    st.markdown('<div class="herti-meta">20+分析维度 · 1次智能生成 · 流式输出 · 全程约1–2分钟</div>', unsafe_allow_html=True)
...
def render_form_page(sb: Optional[Any]) -> None:
    ...
    if st.session_state.pop("_llm_run_gen", False):
        try:
            ...
            status_ph = st.empty()
            preview_ph = st.empty()
            status_ph.info("已连接模型：正在流式生成（先输出结构化摘要，再输出六大模块正文）…")
            ...
            tick = 0
            try:
                for delta in call_llm_backend_stream(
                    backend_url,
                    internal_tok,
                    ...
                    raw_accum += str(delta or "")
                    tick += 1

                    if not saw_summary_end and "<<<END_SUMMARY_JSON>>>" in raw_accum:
                        saw_summary_end = True
                        status_ph.success("摘要结构已就绪：正在流式生成完整 Markdown 报告…")

                    tail = _streaming_report_tail(raw_accum)
                    if saw_summary_end and tail.strip():
                        if tick % 2 == 0 or len(tail) < 220:
                            preview_ph.markdown(
                                '<div class="kai-scope full-report-scope">'
                                + _markdown_to_safe_html(tail)
                                + "</div>",
                                unsafe_allow_html=True,
                            )

                raw = raw_accum.strip() or None
...
<style>
  ...
  /* 完整报告（HTML 渲染） */
  .full-report-scope h3 { font-size: 1.02rem !important; line-height: 1.55 !important; font-weight: 700 !important; color: #7a6328 !important; margin: 14px 0 8px !important; }
  .full-report-scope p,
  .full-report-scope li { font-size: 1.00rem !important; line-height: 1.75 !important; color: #1a1a1a !important; }
...
""",
    ...
    for k in ["市场定位匹配度", "卖点竞争力", "价格带合理性", "冷启动可操作性"]:
        it = dims.get(k) or {}
        dim_rows.append(_format_dimension_line(k, it.get("score", ""), str(it.get("comment", "") or "")))
    dim_html = "".join(
        [
            f"<div style='margin-top:10px;font-size:13px;line-height:1.85;color:var(--text);'>{_esc_html(row)}</div>"
            for row in dim_rows
        ]
    )
    st.markdown(
...
""",
    ...
    act_html = "".join(
        [
            f"<div style='margin-top:10px;font-size:13px;line-height:1.85;color:var(--text);'>• {_esc_html(x)}</div>"
            for x in items[:4]
        ]
    )
    st.markdown(
        f"""
<div class="card kai-scope">
  <div class="card-title report-title-kai-gold">关键行动清单</div>
  {act_html}
</div>
""",
        unsafe_allow_html=True,
    ...
    if (stripe_enabled() or stripe_subscription_enabled()) and not unlocked:
        ...
        preview_raw = full_md[:800] + ("…" if len(full_md) > 800 else "")
        preview_clean = _clean_full_report_markdown(preview_raw)
        preview_html = _markdown_to_safe_html(preview_clean)
        ...
        st.markdown('<div class="full-report-scope">' + preview_html + "</div>", unsafe_allow_html=True)
        ...
        # 默认平铺展示（不折叠）
        st.markdown("<div style='text-align:center;font-size:18px;font-weight:700;color:#7a6328;margin:6px 0 10px 0;'>冷启动全流程深度分析报告</div>", unsafe_allow_html=True)
        report_html = _markdown_to_safe_html(full_md_clean)
        st.markdown('<div class="full-report-scope">' + report_html + "</div>", unsafe_allow_html=True)
    ...
    if st.session_state.get("share_mode"):
        ...
        poster_md_for_export = full_md_clean
        if (stripe_enabled() or stripe_subscription_enabled()) and not unlocked:
            poster_md_for_export = _clean_full_report_markdown(str(full_md[:800] + ("…" if len(full_md) > 800 else "")))
        poster_report_html = _markdown_to_safe_html(poster_md_for_export)
        dim_lines_html = "".join([f"<div>{_esc_html(row)}</div>" for row in dim_rows])
        # 分享长图内容：覆盖「开屏品牌 → 总分/分项 → 分维度 → 行动 → 价格带 → 完整报告（HTML 渲染）」
        poster_body = f"""
<div style="font-family: KaiTi, 'KaiTi_GB2312','STKaiti','Songti SC','Noto Serif SC',serif;">
  <div style="text-align:center;padding:10px 8px 12px;border:1px solid rgba(0,0,0,0.10);border-radius:14px;background:#fff;">
    <div style="color:#8A8276;font-size:11px;letter-spacing:.28em;text-transform:uppercase;">COLD START LAB</div>
    <div style="margin-top:8px;color:#7a6328;font-weight:900;font-size:28px;letter-spacing:.14em;line-height:1.12;">Cold Start Lab</div>
    <div style="margin-top:8px;color:#1a1a1a;font-size:13px;line-height:1.8;">小红书商家冷启动分析</div>
    <div style="margin-top:10px;color:#555;font-size:12px;line-height:1.75;">{_esc_html(str(snap.get('category') or '') or '—')} · {_esc_html(str(snap.get('price_band') or '') or '—')} · {_esc_html(str(snap.get('monthly_budget') or '') or '—')}</div>
  </div>

  <div style="margin-top:14px;color:#7a6328;font-weight:900;font-size:44px;line-height:1;text-align:center;">{int(total100)}/100</div>
  <div style="margin-top:8px;color:#555;font-size:13px;text-align:center;">{_esc_html(rating or '')}</div>

  <div style="margin-top:14px;border-top:1px solid rgba(0,0,0,0.10);padding-top:12px;font-size:13px;line-height:1.85;">
    <div style="color:#7a6328;font-weight:800;margin-bottom:8px;">分数构成</div>
    <div>市场定位匹配度：{_esc_html(str((dims.get("市场定位匹配度") or {}).get("score","")))} /25</div>
    <div>卖点竞争力：{_esc_html(str((dims.get("卖点竞争力") or {}).get("score","")))} /25</div>
    <div>价格带合理性：{_esc_html(str((dims.get("价格带合理性") or {}).get("score","")))} /25</div>
    <div>冷启动可操作性：{_esc_html(str((dims.get("冷启动可操作性") or {}).get("score","")))} /25</div>
  ...
  <div style="margin-top:12px;border-top:1px solid rgba(0,0,0,0.10);padding-top:12px;font-size:13px;line-height:1.85;">
    <div style="color:#7a6328;font-weight:800;margin-bottom:8px;">分维度分析 | KEY POINTS</div>
    {dim_lines_html}
  </div>

  <div style="margin-top:12px;border-top:1px solid rgba(0,0,0,0.10);padding-top:12px;font-size:13px;line-height:1.85;">
    <div style="color:#7a6328;font-weight:800;margin-bottom:8px;">关键行动清单</div>
    {''.join([f"<div>• {_esc_html(x)}</div>" for x in (items[:4] if items else [])])}
  </div>

  <div style="margin-top:12px;border-top:1px solid rgba(0,0,0,0.10);padding-top:12px;font-size:13px;line-height:1.85;">
    <div style="color:#7a6328;font-weight:800;margin-bottom:8px;">价格带 Price Band</div>
    <div>{_esc_html(price_hint)}</div>
  </div>

  <div style="margin-top:12px;color:#777;font-size:11px;text-align:center;">Cold Start Lab · 仅供策略参考 · 可截图保存</div>

  <div style="margin-top:14px;border-top:1px solid rgba(0,0,0,0.10);padding-top:12px;">
    <div style="color:#7a6328;font-weight:900;font-size:16px;text-align:center;letter-spacing:.06em;">冷启动全流程深度分析报告</div>
    <div class="report-md" style="margin-top:10px;">{poster_report_html}</div>
  </div>
</div>
"""
        ...
        st.components.v1.html(comp, height=3200, scrolling=True)

    st.markdown('<div class="footer-note">© 2026 Cold Start Lab · 仅供策略参考</div>', unsafe_allow_html=True)
